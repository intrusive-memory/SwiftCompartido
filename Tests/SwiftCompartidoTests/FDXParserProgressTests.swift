...
struct FDXParserProgressTests {
  ...
  func testAsyncSyncEquivalence() async throws {
    ...
    let nilProgress: OperationProgress? = nil
    let asyncDocument = try await asyncParser.parse(
      data: data, filename: "test.fdx", progress: nilProgress)

    #expect(
      syncDocument.elements.count == asyncDocument.elements.count,
      "Sync and async should produce same element count")

    // Compare element types
    for i in 0..<syncDocument.elements.count {
      #expect(
        syncDocument.elements[i].elementType == asyncDocument.elements[i].elementType,
