...
struct SwiftDataProgressTests {
  ...
  func testConversionProgressDescriptions() async throws {
      ...
      let hasConversion =
        allDescriptions.contains("converting") || allDescriptions.contains("element")
        || allDescriptions.contains("title") || allDescriptions.contains("complete")
  ...
  func testMemoryEfficiency() async throws {
      ...
      #expect(
        updateCount < 200,
        "Should batch updates for memory efficiency (not 600+ individual updates)")
    #else
      throw TestError.swiftDataNotAvailable
    #endif
  }
