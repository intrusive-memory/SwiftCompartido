...
struct GuionTextEditorFormattingTests {
  ...
  func testFormattingApplied() async throws {
    let elements = [
      GuionElementModel(
        elementText: "INT. OFFICE - DAY", elementType: .sceneHeading, orderIndex: 0),
      GuionElementModel(elementText: "JOHN", elementType: .character, orderIndex: 1),
      GuionElementModel(
        elementText: "Hello **world** with *italic*.", elementType: .dialogue, orderIndex: 2),
    ]
  ...
  func testScreenplayMargins() async throws {
    ...
    if let paragraphStyle = attributedText.attribute(.paragraphStyle, at: 0, effectiveRange: nil)
      as? NSParagraphStyle
    {
  ...
  func testMarkdownFormatting() async throws {
    let elements = [
      GuionElementModel(
        elementText: "Chapter One", elementType: .sectionHeading(level: 1), orderIndex: 0),
      GuionElementModel(
        elementText: "Scene One", elementType: .sectionHeading(level: 2), orderIndex: 1),
      GuionElementModel(
        elementText: "- First item", elementType: .unorderedListItem(level: 0), orderIndex: 2),
      GuionElementModel(
        elementText: "1. First number", elementType: .orderedListItem(level: 0), orderIndex: 3),
    ]
  ...
  func testAllElementTypes() async throws {
    ...
    for (text, elementType) in testCases {
      ...
      let attributedText = GuionTextElementMapper.buildAttributedText(from: elements, fontSize: 12)
      #expect(
        attributedText.string.contains(text),
        "Expected to find '\(text)' for element type \(elementType)")
    }
  ...
  func testFontSizeScaling() async throws {
    ...
    print(
      "✅ Font scaling: 8pt=\(fontSize8.length) 16pt=\(fontSize16.length) 24pt=\(fontSize24.length) chars"
    )
  }
  ...
  func testSectionHeadingLevels() async throws {
    let elements = [
      GuionElementModel(
        elementText: "Heading 1", elementType: .sectionHeading(level: 1), orderIndex: 0),
      GuionElementModel(
        elementText: "Heading 2", elementType: .sectionHeading(level: 2), orderIndex: 1),
      GuionElementModel(
        elementText: "Heading 3", elementType: .sectionHeading(level: 3), orderIndex: 2),
      GuionElementModel(
        elementText: "Heading 4", elementType: .sectionHeading(level: 4), orderIndex: 3),
      GuionElementModel(
        elementText: "Heading 5", elementType: .sectionHeading(level: 5), orderIndex: 4),
      GuionElementModel(
        elementText: "Heading 6", elementType: .sectionHeading(level: 6), orderIndex: 5),
    ]
  ...
  func testListIndentation() async throws {
    let elements = [
      GuionElementModel(
        elementText: "- Level 0", elementType: .unorderedListItem(level: 0), orderIndex: 0),
      GuionElementModel(
        elementText: "- Level 1", elementType: .unorderedListItem(level: 1), orderIndex: 1),
      GuionElementModel(
        elementText: "1. Number 0", elementType: .orderedListItem(level: 0), orderIndex: 2),
      GuionElementModel(
        elementText: "1. Number 1", elementType: .orderedListItem(level: 1), orderIndex: 3),
    ]
