...
struct MarkdownViewTests {
  ...
  func testMarkdownListItemViewNumbering() throws {
    ...
    let elements = [
      makeElement(
        in: container.mainContext, text: "First", type: .orderedListItem(level: 0), order: 0),
      makeElement(
        in: container.mainContext, text: "Second", type: .orderedListItem(level: 0), order: 1),
      makeElement(
        in: container.mainContext, text: "Third", type: .orderedListItem(level: 0), order: 2),
    ]
  ...
  func testMarkdownListItemReferenceViewInitialization() throws {
    let container = try makeTestContainer()
    let element = makeElement(
      in: container.mainContext, text: "Reference list item", type: .unorderedListItem(level: 0))
  ...
  func testMarkdownViewsIntegration() throws {
    ...
    let elements = [
      makeElement(
        in: container.mainContext, text: "# Main Title", type: .sectionHeading(level: 1), order: 0),
      makeElement(
        in: container.mainContext, text: "Introduction paragraph with **bold**.", type: .action,
        order: 1),
      makeElement(
        in: container.mainContext, text: "## Subsection", type: .sectionHeading(level: 2), order: 2),
      makeElement(
        in: container.mainContext, text: "First item", type: .orderedListItem(level: 0), order: 3),
      makeElement(
        in: container.mainContext, text: "Second item", type: .orderedListItem(level: 0), order: 4),
    ]
    let document = makeDocument(in: container.mainContext, withElements: elements)
  ...
  func testMarkdownViewsComplexNesting() throws {
    ...
    let elements = [
      makeElement(
        in: container.mainContext, text: "# Chapter 1", type: .sectionHeading(level: 1), order: 0),
      makeElement(in: container.mainContext, text: "Introduction", type: .action, order: 1),
      makeElement(
        in: container.mainContext, text: "Main point", type: .unorderedListItem(level: 0), order: 2),
      makeElement(
        in: container.mainContext, text: "Subpoint", type: .unorderedListItem(level: 1), order: 3),
      makeElement(
        in: container.mainContext, text: "Sub-subpoint", type: .unorderedListItem(level: 2),
        order: 4),
      makeElement(
        in: container.mainContext, text: "## Section 1.1", type: .sectionHeading(level: 2), order: 5
      ),
      makeElement(
        in: container.mainContext, text: "Details with *italic*", type: .action, order: 6),
    ]
    let document = makeDocument(in: container.mainContext, withElements: elements)
