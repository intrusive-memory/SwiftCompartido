...
@Test func testGroupScenesByLocation() async throws {
  ...
  for (_, group) in groups {
    ...
    #expect(
      !group.representativeLocation.scene.isEmpty,
      "Each group should have a representative location")
  }
  ...
  if let multiSceneGroup = multiSceneGroups.first {
    print(
      "Location '\(multiSceneGroup.representativeLocation.fullLocation)' has \(multiSceneGroup.sceneCount) scenes"
    )
...
@Test func testGetLocationsByFrequency() async throws {
  ...
  for (index, group) in locationsByFrequency.prefix(5).enumerated() {
    print(
      "  \(index + 1). \(group.representativeLocation.fullLocation) - \(group.sceneCount) scenes")
  }
}
...
@Test func testLightingAndTimeAnalysis() async throws {
  ...
  if !mixedLightingGroups.isEmpty {
    ...
    for group in mixedLightingGroups.prefix(3) {
      print(
        "  \(group.representativeLocation.fullLocation): \(group.lightingTypes.map { $0.standardAbbreviation })"
      )
    }
  }
...
@Test func testProductionScenario() async throws {
  ...
  let coffeeShopGroup = groups.values.first {
    $0.representativeLocation.scene.uppercased() == coffeeShopKey
  }
