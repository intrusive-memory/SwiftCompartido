...
struct OutlineLevelParsingTests {
  ...
  @Test func testMultipleLevel1HeadersWithSeparateHierarchies() async throws {
    ...
    #expect(
      allLevel2.allSatisfy { $0.parentId == pitchNotesId || $0.parentId == screenplayId },
  ...
  @Test func testMissingLevel3CreatesSymtheticElement() async throws {
    ...
    #expect(
      syntheticLevel3?.parentId == chapter?.id, "Synthetic level 3 should be child of chapter")
  ...
  @Test func testMissingLevel2And3CreatesMultipleSyntheticElements() async throws {
    ...
    #expect(
      syntheticLevel3?.parentId == syntheticLevel2?.id,
      "Synthetic level 3 should be child of synthetic level 2")
    #expect(scene?.parentId == syntheticLevel3?.id, "Scene should be child of synthetic level 3")
  }
  ...
  @Test func testMultipleScenesWithMissingLevel3() async throws {
    ...
    #expect(
      level3Elements.count == 1, "Should create only 1 synthetic level 3 element for both scenes")
    ...
    #expect(syntheticLevel3?.isSynthetic ?? false, "Level 3 should be marked as synthetic")
    #expect(
      level4Elements.allSatisfy { $0.parentId == syntheticLevel3?.id },
      "Both scenes should share the same synthetic parent")
  }
  ...
  @Test func testMissingLevelWhenSceneGroupExists() async throws {
    ...
    #expect(
      syntheticElements.count == 0, "Should not create synthetic level 3 when it already exists")

    // Both the scene heading and the #### element should be level 4 and be children of the explicit level 3
    let level4Elements = outline.filter { $0.level == 4 }
    #expect(
      level4Elements.count == 2, "Should have 2 level 4 elements (scene heading + #### section)")

    let explicitLevel3 = level3Elements.first { !$0.isSynthetic }
    #expect(
      level4Elements.allSatisfy { $0.parentId == explicitLevel3?.id },
      "All level 4 elements should be children of explicit level 3")
  }
