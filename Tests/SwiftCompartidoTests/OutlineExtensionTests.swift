...
struct OutlineExtensionTests {
  ...
  func testOutlineWithNoLevelOneHeader() async throws {
    let fountainURL = try await FixtureManager.shared.withExclusiveAccess(to: "bigfish.fountain") {
      $0
    }
  ...
  func testSceneDirectiveParsing() async throws {
    ...
    for directive in sceneDirectives {
      #expect(
        directive.sceneDirective != nil, "Scene directive should have sceneDirective property")
  ...
  func testElementTypePropertyReturnsOutline() async throws {
    ...
    for element in outline {
      #expect(
        element.elementType == "outline", "All outline elements should have elementType 'outline'")
    }
    ...
    if let sceneHeader = outline.first(where: { $0.type == "sceneHeader" }) {
      #expect(
        sceneHeader.elementType == "outline",
        "Scene header element should have elementType 'outline'")
    }
    ...
    for header in sectionHeaders {
      #expect(
        header.elementType == "outline", "All section headers should have elementType 'outline'")
    }
  }
  ...
  func testParentChildRelationships() async throws {
    ...
    for element in outline {
      ...
      #expect(
        children.count == element.childIds.count, "children() count should match childIds count")
    }
  }
  ...
  func testConvenienceTreeExtraction() async throws {
    ...
    #expect(
      tree.allNodes.count == manualTree.allNodes.count,
      "Convenience method should produce same tree structure")
