...
@testable import SwiftCompartido
  ...
  struct FDXDocumentWriterTests {
    ...
    func testFDXRoundTrip() throws {
      ...
      // Verify content
      #expect(
        parsedDocument.elements.count == elements.count, "Should have same number of elements")

      for (index, (expectedType, expectedText)) in elements.enumerated() {
        let parsedElement = parsedDocument.elements[index]
        #expect(
          parsedElement.elementType == ElementType(string: expectedType),
          "Element type should match at index \(index)")
        #expect(
          parsedElement.elementText == expectedText, "Element text should match at index \(index)")
      }
    }
