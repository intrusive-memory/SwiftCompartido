...
struct GeneratedContentSortingTests {
  ...
  func testSortedElementGeneratedContent() throws {
    ...
    #expect(
      sorted[1].prompt.contains("Coffee shop") || sorted[1].mimeType.hasPrefix("image/"),
      "Second should be chapter 1 scene (1,2)")
    #expect(
      sorted[2].prompt == "Hello, how are you?" || sorted[2].mimeType.hasPrefix("audio/"),
      "Third should be chapter 1 dialogue (1,3)")
    #expect(
      sorted[3].mimeType == "application/x-embedding",
      "Fourth should be chapter 2 scene embedding (2,2)")
  ...
  func testFilterByMimeType() throws {
    ...
    let embeddingContent = document.sortedElementGeneratedContent(
      mimeTypePrefix: "application/x-embedding")
