...
struct ElementProgressBarTests {
  ...
  private func makeTestElement(in context: ModelContext, text: String = "Test element")
    -> GuionElementModel
  {
  ...
  func testElementProgressBarShowsError() throws {
    ...
    #expect(progressState.hasVisibleProgress(for: element.persistentModelID) == true)
    #expect(
      progressState.progress(for: element.persistentModelID)?.message?.contains("Failed to process")
        == true)
  }
  ...
  func testElementProgressBarMultipleElements() throws {
    ...
    #expect(
      progressState.progress(for: element1.persistentModelID)?.message == "Element 1 progress")
    #expect(
      progressState.progress(for: element2.persistentModelID)?.message == "Element 2 progress")
  }
