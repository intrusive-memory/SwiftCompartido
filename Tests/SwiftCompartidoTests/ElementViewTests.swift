...
struct ElementViewTests {
  ...
  func testActionViewMargins() {
    let element = createMockElement(
      type: .action, text: "Action with multi-line text that should wrap properly.")
    let view = ActionView(element: element)
      .environment(\.screenplayFontSize, 12)
  ...
  func testSectionHeadingViewFontScaling() {
    for level in 1...6 {
      let element = createMockElement(
        type: .sectionHeading(level: level), text: "Heading Level \(level)")
      let view = SectionHeadingView(element: element)
        .environment(\.screenplayFontSize, 12)
  ...
  func testSynopsisViewMultiLine() {
    let longText =
      "This is a longer synopsis that describes the scene in detail and may span multiple lines."
  ...
  func testCommentViewLongText() {
    let longComment =
      "This is a very long comment that provides detailed notes about the scene and may contain important production information."
