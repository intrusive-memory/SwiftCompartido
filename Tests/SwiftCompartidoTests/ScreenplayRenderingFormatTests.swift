...
struct ScreenplayRenderingFormatTests {
  ...
  func testCharacterLeftMargin() {
    ...
    let expectedLeftMargin =
      expectedCharacterWidth * (ScreenplayPageFormat.charactersPerLine * 0.40)

    // Character uses 60% width for content
    let expectedContentWidth =
      expectedCharacterWidth * (ScreenplayPageFormat.charactersPerLine * 0.60)
  ...
  func testDialogueMargins() {
    ...
    // Dialogue: 25% left margin, 50% width
    let expectedLeftMargin =
      expectedCharacterWidth * (ScreenplayPageFormat.charactersPerLine * 0.25)
    let expectedContentWidth =
      expectedCharacterWidth * (ScreenplayPageFormat.charactersPerLine * 0.50)
  ...
  func testParentheticalMargins() {
    ...
    // Parenthetical: 32% left margin, 38% width
    let expectedLeftMargin =
      expectedCharacterWidth * (ScreenplayPageFormat.charactersPerLine * 0.32)
    let expectedContentWidth =
      expectedCharacterWidth * (ScreenplayPageFormat.charactersPerLine * 0.38)
  ...
  func testTransitionMargins() {
    ...
    let expectedLeftMargin =
      expectedCharacterWidth * (ScreenplayPageFormat.charactersPerLine * 0.65)
    let expectedContentWidth =
      expectedCharacterWidth * (ScreenplayPageFormat.charactersPerLine * 0.35)
  ...
  func testFontSizeResponsiveness() {
    ...
    for fontSize in fontSizes {
      for (type, text) in elementTypes {
        ...
        case .character:
          let _ = DialogueCharacterView(element: element).environment(
            \.screenplayFontSize, fontSize)
        case .dialogue:
          let _ = DialogueTextView(element: element).environment(\.screenplayFontSize, fontSize)
        case .parenthetical:
          let _ = DialogueParentheticalView(element: element).environment(
            \.screenplayFontSize, fontSize)
        case .transition:
  ...
  func testMarginRatiosAcrossFontSizes() {
    ...
    for fontSize in fontSizes {
      ...
      #expect(
        characterMargin > dialogueMargin,
        "Character margin (40%) should be greater than dialogue margin (25%)")
      #expect(
        transitionMargin > characterMargin,
        "Transition margin (65%) should be greater than character margin (40%)")
      ...
      #expect(
        dialogueToCharacterRatio > 1.5 && dialogueToCharacterRatio < 1.7,
        "Ratio should be ~1.6 (40/25)")
    }
  }
}
