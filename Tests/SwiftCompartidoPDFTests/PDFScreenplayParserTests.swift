...
struct PDFScreenplayParserTests {
  ...
  func testTextExtraction() async throws {
    ...
    print(
      "✅ BULLITT.pdf - Scenes: \(sceneHeadings.count), Dialogue: \(dialogue.count), Action: \(action.count)"
    )
  }
  ...
  func testProgressReporting() async throws {
    ...
    let hasExtraction = descriptions.contains {
      $0.contains("Extracting") || $0.contains("Reading")
    }
    let hasConversion = descriptions.contains {
      $0.contains("Converting") || $0.contains("formatting")
    }
    ...
    print(
      "   Phases - Extraction: \(hasExtraction), Conversion: \(hasConversion), Parsing: \(hasParsing)"
    )
  }
  ...
  func testContentPreservation() async throws {
    ...
    let emptyElements = screenplay.elements.filter {
    ...
    print(
      "✅ Content validation - \(screenplay.elements.count) elements, \(emptyElements.count) empty (\(String(format: "%.1f", emptyPercentage * 100))%)"
    )
  }
  ...
  func testContentPreservationThroughConversion() async throws {
    ...
    print(
      "✅ Content preserved - \(elementsWithText.count)/\(screenplay.elements.count) elements with text (\(String(format: "%.1f", contentPercentage * 100))%)"
    )
  ...
  func testPageNumberFiltering() async throws {
    ...
    #expect(
      numberPercentage < 0.05,
      "Should filter most page numbers (found \(elementsWithJustNumbers.count))")

    print(
      "✅ Page number filtering works - \(elementsWithJustNumbers.count)/\(screenplay.elements.count) standalone numbers (\(String(format: "%.1f", numberPercentage * 100))%)"
    )
  }
  ...
  func testMultiplePageConversion() async throws {
    ...
    print(
      "✅ Multi-page conversion - \(screenplay.elements.count) elements, \(sceneHeadings.count) scenes"
    )
  }
  ...
  func testDialogueFormatting() async throws {
    ...
    print(
      "✅ Dialogue formatting - \(meaningfulDialogue.count)/\(dialogue.count) meaningful (\(String(format: "%.1f", dialogueQuality * 100))%)"
    )
  }
  ...
  func testSceneContinuity() async throws {
    ...
    let sceneHeadings = screenplay.elements.enumerated().filter {
      $0.element.elementType == .sceneHeading
    }
    ...
    for (index, (position, scene)) in sceneHeadings.enumerated() {
      let nextScenePosition =
        sceneHeadings.indices.contains(index + 1)
  ...
  func testSceneHeadingFormat() async throws {
    ...
    print(
      "✅ Scene heading format: \(validFormatCount)/\(sceneHeadings.count) valid (\(String(format: "%.1f", formatAccuracy * 100))%)"
    )
  }
