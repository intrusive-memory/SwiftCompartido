...
public struct ElementFilter: Sendable, Codable, Equatable {
  ...
  public var isEmpty: Bool {
    elementTypes == nil && chapterIndex == nil && characterName == nil && searchText == nil
  }
    ...
    for element in elements {
      ...
      if elementMatches {
        ...
        case .dialogue, .parenthetical, .lyrics:
          ...
          elementMatches =
            currentCharacter?.localizedCaseInsensitiveCompare(targetCharacter) == .orderedSame
        default:
          // For non-dialogue elements, character filter doesn't apply
          // Only include if not filtering to dialogue-only
          if let types = elementTypes,
            types.contains(.dialogue) || types.contains(.parenthetical) || types.contains(.lyrics)
          {
