...
public struct GuionDocumentSnapshot: Codable, Identifiable, Sendable {
    ...
    get {
      ...
      for (index, rawJSON) in data.enumerated() {
        ...
        guard let dict = try? JSONSerialization.jsonObject(with: rawJSON) as? [String: Any] else {
          logger.warning(
            "Failed to deserialize custom page at index \(index): Invalid JSON data (size: \(rawJSON.count) bytes). This page will be lost if the document is saved."
          )
          continue
        }
        ...
        do {
          ...
          logger.warning(
            "Failed to deserialize custom page at index \(index): \(error.localizedDescription). This page will be lost if the document is saved."
          )
        }
      }
  ...
  public mutating func setVoiceMapping(
    _ mapping: CharacterVoiceMappingSnapshot, for character: String
  ) {
    if casting == nil {
      casting = [:]
    }
