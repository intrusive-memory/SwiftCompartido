...
extension GuionParsedElementCollection {
    ...
    if title == nil {
      ...
      for titlePageSection in titlePage {
        if let titleArray = titlePageSection["Title"] ?? titlePageSection["title"] {
          if let firstTitle = titleArray.first,
            !firstTitle.trimmingCharacters(in: .whitespaces).isEmpty
          {
  ...
  private func buildSceneGroupData(sceneGroup: OutlineElement, outline: OutlineList)
    -> SceneGroupData
  {
  ...
  private func collectNestedScenes(under parent: OutlineElement, in outline: OutlineList)
    -> [OutlineElement]
  {
  ...
  private func buildScenesWithOverBlack(sceneElements: [OutlineElement], outline: OutlineList)
    -> [SceneData]
  {
