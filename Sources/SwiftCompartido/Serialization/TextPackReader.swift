...
public struct TextPackReader {
  ...
  public static func readTextPack(from fileWrapper: FileWrapper) throws
    -> GuionParsedElementCollection
  {
    ...
    guard let screenplayWrapper = fileWrappers["screenplay.fountain"],
      ...
      let screenplayContent = String(data: screenplayData, encoding: .utf8)
    else {
    ...
    public static func readTextPack(
      ...
      return await GuionDocumentModel.from(
        screenplay, in: context, generateSummaries: generateSummaries)
    }
  #endif
  ...
  public static func readCharacters(from fileWrapper: FileWrapper) -> TextPackCharacterList? {
    guard let resourcesWrapper = fileWrapper.fileWrappers?["Resources"],
      ...
      let data = charactersWrapper.regularFileContents
    else {
      return nil
    }
  ...
  public static func readLocations(from fileWrapper: FileWrapper) -> LocationList? {
    guard let resourcesWrapper = fileWrapper.fileWrappers?["Resources"],
      ...
      let data = locationsWrapper.regularFileContents
    else {
      return nil
    }
  ...
  public static func readElements(from fileWrapper: FileWrapper) -> ElementList? {
    guard let resourcesWrapper = fileWrapper.fileWrappers?["Resources"],
      ...
      let data = elementsWrapper.regularFileContents
    else {
      return nil
    }
  ...
  public static func readTitlePage(from fileWrapper: FileWrapper) -> TitlePageData? {
    guard let resourcesWrapper = fileWrapper.fileWrappers?["Resources"],
      ...
      let data = titlePageWrapper.regularFileContents
    else {
      return nil
    }
