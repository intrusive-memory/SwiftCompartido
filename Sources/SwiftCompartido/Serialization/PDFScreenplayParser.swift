...
public enum PDFScreenplayParserError: Error, LocalizedError {
  ...
  public var errorDescription: String? {
    ...
    case .textExtractionFailed:
      return
        "Failed to extract text from PDF. The PDF may contain only images (OCR not yet supported)."
    case .foundationModelsUnavailable:
      return
        "Foundation Models not available. This feature requires iOS 26+/macOS 26+ with Apple Intelligence enabled."
...
public final class PDFScreenplayParser {
  ...
  public static func parse(
    ...
    progress?.update(
      completedUnits: 20, description: "Text extracted from \(pages.count) pages", force: true)
    ...
    progress?.update(
      completedUnits: 20, description: "Converting to screenplay format...", force: true)
    ...
    for pageIndex in 0..<pageCount {
      ...
      if !pageContent.plainText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
        pages.append(pageContent)
          ...
          print(
            "📄 [PDFParser] Page \(pageIndex + 1): \(charCount) chars plain, \(formattedCharCount) chars formatted (~\(tokenEstimate) tokens)"
          )
        #endif
      }
  ...
  private static func convertPagesToFountain(
    ...
    progress: OperationProgress?
      ...
      for (index, pageContent) in pages.enumerated() {
        ...
        let pageProgress =
          baseProgress + Int64((Double(rangeSize) * Double(pageNumber)) / Double(totalPages))
        progress?.update(
          completedUnits: pageProgress,
      ...
      if failureRate > 0.5 {
        ...
        progress?.update(
          ...
          additionalInfo:
            "⚠️ AI-powered parsing unavailable for this document. Falling back to heuristic conversion. Results may be less accurate for non-standard screenplay formats.",
          force: true
        )
      ...
      #if DEBUG
        print(
          "ℹ️ [PDFParser] convertPagesToFountain: FoundationModels NOT available at compile time")
      ...
      progress?.update(
        ...
        additionalInfo:
          "ℹ️ Apple Intelligence not available on this platform. Using heuristic conversion. For AI-powered parsing, upgrade to iOS 26.2+ or macOS 26.0+ with M1+/A17 Pro+ chip.",
        force: true
      )
      return await convertAllPagesToFountainBasic(pages, progress: progress)
    #endif
  ...
  private static func convertPageToFountainWithAI(
    _ pageContent: PDFPageContent
      ...
      #if DEBUG
        print(
          "🤖 [PDFParser] Page \(pageContent.pageNumber): Attempting AI conversion with formatting")
        let charCount = pageContent.formattedText.count
        let tokenEstimate = charCount / 3
        print(
          "🤖 [PDFParser] Page \(pageContent.pageNumber): \(charCount) chars formatted (~\(tokenEstimate) tokens)"
        )
      #endif
      ...
      do {
        let response = try await session.respond(to: Prompt(userPrompt))
          ...
          print(
            "✅ [PDFParser] Page \(pageContent.pageNumber): Output \(response.content.count) chars")
        #endif
