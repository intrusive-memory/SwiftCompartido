  ...
  public init(configuration: ReadConfiguration) throws {
      ...
      let data = configuration.file.regularFileContents
    {
      do {
        ...
        #if DEBUG
          print(
            "📄 JSON .guion file loaded: \(configuration.file.filename ?? "unknown"), \(snapshot.elements.count) elements"
          )
        #endif
        return
      } catch {
        #if DEBUG
          print(
            "⚠️ Failed to decode as JSON, trying TextPack fallback: \(error.localizedDescription)")
        #endif
    ...
    if configuration.file.isDirectory,
      configuration.contentType == .guionDocument
        || configuration.file.filename?.hasSuffix(".guion") == true
    {
    ...
    let content = Self.extractContent(
      from: configuration.file, filename: configuration.file.filename)
    #if DEBUG
      print(
        "📥 Document loaded: \(configuration.file.filename ?? "unknown"), content length: \(content?.count ?? 0)"
      )
    #endif
    document.rawContent = content
    document.filename = configuration.file.filename
  ...
  private static func extractHighlandContent(from zipData: Data) -> String? {
    ...
    do {
      ...
      let textBundleContents = try fileManager.contentsOfDirectory(
        at: textBundleURL, includingPropertiesForKeys: nil)
      ...
      if let fountainURL = textBundleContents.first(where: {
        $0.pathExtension.lowercased() == "fountain"
      }) {
        #if DEBUG
  ...
  public static func parseContent(
    ...
    if contentType == .guionDocument, let data = rawContent.data(using: .utf8) {
      do {
        ...
        #if DEBUG
          print(
            "⚠️ Failed to parse as JSON .guion, falling back to Fountain parsing: \(error.localizedDescription)"
          )
        #endif
