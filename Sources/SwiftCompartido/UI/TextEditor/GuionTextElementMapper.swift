...
public class GuionTextElementMapper {
    ...
    public static func buildAttributedText(
        ...
        let baseFont = PlatformFont(name: "Courier New", size: fontSize) ?? PlatformFont.monospacedSystemFont(ofSize: fontSize, weight: .regular)
    ...
    private static func createFormattedElement(
        ...
        if let formattedText = element.formattedText {
            ...
            applyElementFormatting(to: mutableText, element: element, fontSize: fontSize, charWidth: charWidth)
            return mutableText
        } else {
    ...
    private static func applyInlineFormatting(
        ...
        // Bold: **text**
        applyPattern("\\*\\*([^*]+)\\*\\*", to: attrString, baseFont: baseFont, fontSize: fontSize, isBold: true, isItalic: false)

        // Italic: *text*
        applyPattern("(?<!\\*)\\*([^*]+)\\*(?!\\*)", to: attrString, baseFont: baseFont, fontSize: fontSize, isBold: false, isItalic: true)
    ...
    private static func applyPattern(
        ...
        for match in matches.reversed() {
            ...
            if let descriptor = baseFont.fontDescriptor.withSymbolicTraits(UIFontDescriptor.SymbolicTraits(rawValue: symbolicTraits)) {
            ...
            #elseif os(macOS)
            let font: PlatformFont
            let descriptor = baseFont.fontDescriptor.withSymbolicTraits(NSFontDescriptor.SymbolicTraits(rawValue: symbolicTraits))
    ...
    private static func applyUnderlinePattern(_ pattern: String, to attrString: NSMutableAttributedString) {
        guard let regex = try? NSRegularExpression(pattern: pattern) else { return }
    ...
    private static func applyElementFormatting(
        ...
        let paragraphStyle = createParagraphStyle(for: element.elementType, charWidth: charWidth, fontSize: fontSize)
    ...
    private static func createFontAndColor(for elementType: ElementType, baseFontSize: CGFloat) -> (PlatformFont, PlatformColor) {
        switch elementType {
        case .sceneHeading:
            // Bold, 1.5x size, uppercase
            let size = baseFontSize * 1.5
            let font = PlatformFont(name: "Courier New Bold", size: size)
        ...
        case .comment:
            // Gray, italic
            let font = PlatformFont(name: "Courier New Italic", size: baseFontSize)
        ...
        case .boneyard:
            // Strikethrough, gray
            let font = PlatformFont(name: "Courier New", size: baseFontSize)
                ?? PlatformFont.monospacedSystemFont(ofSize: baseFontSize, weight: .regular)
            #if os(iOS)
        ...
        case .pageBreak:
            // Centered, gray
            let font = PlatformFont(name: "Courier New Bold", size: baseFontSize)
