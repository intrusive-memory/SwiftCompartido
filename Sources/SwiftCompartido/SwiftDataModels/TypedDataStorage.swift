...
public enum TypedDataStorageError: Error, CustomStringConvertible {
  ...
  public var description: String {
    ...
    case .unsupportedMimeType(let mimeType):
      return
        "Unsupported MIME type: \(mimeType). Supported types: text/*, image/*, audio/*, video/*, application/x-embedding"
...
public final class TypedDataStorage {
  ...
  public static func isMimeTypeSupported(_ mimeType: String) -> Bool {
    ...
    return lowercased.hasPrefix("text/") || lowercased.hasPrefix("image/")
      || lowercased.hasPrefix("audio/") || lowercased.hasPrefix("video/")
  ...
  public func getContent(
    from storageArea: StorageAreaReference? = nil, progress: OperationProgress? = nil
  ) throws -> Data {
    ...
    if mimeType.lowercased().hasPrefix("text/") {
      ...
      if let textValue = textValue {
        ...
        progress?.update(
          completedUnits: Int64(data.count), description: "Loaded text from memory", force: true)
        return data
      }
    } else {
      // Binary content
      if let binaryValue = binaryValue {
        ...
        progress?.update(
          completedUnits: Int64(binaryValue.count), description: "Loaded binary from memory",
          force: true)
        return binaryValue
      }
    }
  ...
  private func loadFileWithProgress(
    ...
    if fileSize > chunkSize, let progress = progress {
      ...
      // Report completion
      progress.update(
        completedUnits: fileSize, description: "Loaded \(fileRef.fileName)", force: true)
      return data
    } else {
...
extension TypedDataStorage: CustomStringConvertible {
  public var description: String {
    ...
    return
      "TypedDataStorage(id: \(id), type: \(contentCategory), mime: \(mimeType)\(details), size: \(size), storage: \(storage), sync: \(sync))"
  }
}
